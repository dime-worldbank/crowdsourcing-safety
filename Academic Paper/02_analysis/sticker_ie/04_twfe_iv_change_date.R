...
coef_all_df <- map_df(date_seq, function(date_i){
  print(date_i)
  ...
  coef_df <- map_df(y_vars, function(y_var){
    ...
    sensor_df <- sensor_df %>%
      ...
      mutate(days_since_stk_install_post_assign = case_when(
        (date >= ymd(date_i)) & (sticker_group_not_installed == 1) ~ 1,
...
for(est_type_i in c("ITT", "ATT")){
  ...
  p <- coef_all_df %>%
    ...
    theme(strip.background = element_blank(),
          ...
          axis.title = element_text(size = 10, angle = 0, vjust = 0.5)) 
  
  ggsave(p,
         filename = file.path(figures_dir, 
                              paste0("sticker_ie_coefs_change_date_",tolower(est_type_i),".png")),
         height = 4.5, width = 10)
}
