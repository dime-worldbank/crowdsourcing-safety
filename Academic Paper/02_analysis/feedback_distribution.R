...
p_speed_alt <- veh_stack_df %>%
  dplyr::filter(name == "q_speed_rating_alt_num") %>%
  ggplot() +
  geom_col(aes(x = value,
               y = n),
           width = COL_WIDTH,
...
#   labs(x = NULL,
#        y = "N Vehicles",
#        title = "E. Vehicle average of: comment sentiment") +
#   custom_theme
...
p_prop_speed_alt <- veh_stack_df %>%
  dplyr::filter(name == "q_speed_rating_alt_dfast") %>%
  ggplot() +
  geom_col(aes(x = value,
               y = n),
           #width = COL_WIDTH,
...
p_prop_driving_snmtr_neg <- veh_stack_sntmt_df %>%
