...
for(type in c("main", "wunknown")){
  for(comment_filter in c(TRUE, FALSE)){
    for(distinct_pass in c(TRUE, FALSE)){
      ...
      cross_tab_df <- table(fb_df$q_safety_rating, fb_df$q_speed_rating_v2) %>%
      ...
      # Make table -------------------------------------------------------------------
      file_name <- paste0("safe_speed_v2_crosstab_",type,
                          "_cmntfilter", comment_filter,
                          "_dstnctpass", distinct_pass, ".tex")
      
      # if(type == "main")     file_name <- "safe_speed_v2_crosstab.tex"
      # if(type == "wunknown") file_name <- "safe_speed_v2_crosstab_wunknown.tex"
