...
for(type in c("main", "wunknown")){
  ...
  if(type == "wunknown"){
    df <- readRDS(file.path(data_dir, "FinalData", "passenger_feedback_valid_wunknown.Rds"))
  }
  ...
  if(type == "wunknown"){
    saveRDS(df, file.path(data_dir, "FinalData", "passenger_feedback_valid_class_wunknown.Rds"))
  }
