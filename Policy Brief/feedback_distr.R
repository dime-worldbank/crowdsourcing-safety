...
# Load data --------------------------------------------------------------------
veh_df <- readRDS(file.path(ap_data_dir, "FinalData", "vehicle_level_stickers_suff_feedback.Rds"))
...
p_level <- ggarrange(p_safe, p_speed_2, ncol = 1)
...
p_prop <- ggarrange(p_prop_safe, p_prop_speed_v2, ncol = 1)

# Arrange/Export ---------------------------------------------------------------
p <- ggarrange(p_level, p_prop)

ggsave(p, filename = file.path(brief_figures_dir, "pass_rating_distribution_brief.png"),
       height = 3, width = 6)
